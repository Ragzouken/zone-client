...
export function makeGlyph(position: Vector2, 
                          ...
                          hidden = true,
                          styles = new Map<string, unknown>()): Glyph
{
...
export function scriptToPages(script: string, 
...
function find<T>(array: T[], 
                 start: number, 
                 step: number, 
                 predicate: ArrayPredicate<T>): [T, number] | undefined
{
...
export function commandsToPages(commands: Command[],
                                layout: LayoutContext,
                                styleHandler: StyleHandler = defaultStyleHandler): Page[]
{
 ...
 */
export function commandsBreakLongSpans(commands: Command[], 
                                       context: LayoutContext): void
{
    const canBreak = (command: Command) => command.type === "break" 
